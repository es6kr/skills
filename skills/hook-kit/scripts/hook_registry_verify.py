...
def _atomic_write(path: str, render) -> None:
    ...
    open(path, "w") truncates before a single byte is written, so an
    interrupt or a serialization error mid-write leaves the canonical
    registry — or the live hooks.json — truncated. A truncated hooks.json
    silently disables every guard in that marketplace, which is the same
    "exit 127 reads as no objection" failure this tool exists to catch,
    arriving by a different route. os.replace is atomic on POSIX and
    Windows, so a reader sees either the old file or the new one.
    """
    directory = os.path.dirname(path) or "."
    os.makedirs(directory, exist_ok=True)
    fd, tmp = tempfile.mkstemp(dir=directory, prefix=".hrv-", suffix=".tmp")
    try:
        ...
        # Leave the original file untouched; drop the partial temp.
        try:
            os.unlink(tmp)
        except OSError:
            pass
