...
| `block-tasklist-id-in-conversation.sh` (lives in `todowrite/resources/`) | (a) `#NN` standalone (without PR/issue prefix) (b) **each distinct** PR number referenced (`PR #N` / `PR N`) that has no URL ending in `/pull/<that same N>` in the questions payload — one PR's URL does NOT cover a different PR number | (a) Use `PR #NN` / `issue #NN` prefix or subject keyword (e.g., "Web-PR-346 verification") (b) include the full `https://github.com/<o>/<r>/pull/<N>` URL **per referenced PR number**, in the question text or an option description |
...
| `block-commit-tag-change-without-package-count.sh` | Option proposes changing a commit's Conventional Commit tag (`reword`/`retag`/`squash`/amend-message verbs, or two CC tags joined by a change marker like `fix: -> feat:` / `fix(scope): to feat(scope):`) in a path-scoped monorepo (`.release-please-manifest.json` with ≥2 packages) | Run a real per-commit file enumeration (`git diff-tree --name-only\|--name-status\|--stat -r <sha>` or `git show --name-only\|--name-status\|--stat <sha>`) as an actual Bash command **in the current turn** before proposing the retag, then state the per-commit package count in the option description; or set `COMMIT_TAG_PACKAGE_COUNT_VERIFIED=1` when the counts are already verified |
...
1. Visually grep the entire label + description against the 7 hook block patterns above
...
2. Iterate the 7 hook block patterns above and grep
