...
depends-on:
  - archive
  - safe-delete
description: "Hook management. audit - reference/permission/orphan checks (stale references + chmod +x missing + unowned hook detection) [audit.md]. edit - hook script modification + dual-sync [edit.md]. install - install from resources/ to hooks/ + settings.json registration [install.md]. move - move hooks from scripts/ to hooks/ + update settings.json paths [move.md]. remove - remove hook entries from settings.json [remove.md]. registry - canonical hook inventory hook-registry.yaml now lives in a private companion repository (relocated 2026-09-10); consult before any registration change [registry.md]. Use when: \"hook cleanup\", \"hook audit\", \"hook edit\", \"hook install\", \"hook move\", \"hook remove\", \"install hook\", \"edit hook\", \"edit guard\", \"bash-guard\", \"hook sync\", \"hook permission\", \"orphan hook\", \"chmod +x hook\", \"exit 126\", \"hook registry\", \"hook-registry.yaml\", \"phantom hook\", \"ghost hook\", \"hook registration\", \"hooks.json\", \"settings.json hooks\""
allowed-tools:
  - Read
  - Edit
  - Grep
