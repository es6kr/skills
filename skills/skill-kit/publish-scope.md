...
Before a new skill directory enters a public catalog commit, all three axes must pass **per skill**:
...
| # | Don't | Do |
|---|-------|-----|
| 1 | Bundle every "clearly my own" untracked skill into one wholesale catalog commit because a backlog issue says "commit the backlog" | Run the three-axis gate per skill; commit only the skills that pass all three, and report the held ones with the failing axis |
...
| 3 | Treat CI/lint green as publication readiness | Lint proves formatting, not slug intent, dedup state, or scope fit. The three axes are separate from CI |
