...
- Direct call via `/fa` (this skill's Procedure Step 2 follows this topic)
- Auto-run during `/cleanup run` Step 2 — the cleanup pipeline reaches this topic through
  `Skill("claudify", "improve")`, whose retrospect component invokes `Skill("fa")`
- When a caller's args contain mistake/improvement-related keywords (e.g., "improve foolishness", "record mistake", "save feedback")
...
5. **Conflict check (cross-class contradiction — separate from same-class recurrence)**: Stage 1's grep only asks "does this exact pattern already exist" (same-class recurrence). It does not ask "does this new entry's conclusion contradict a *different* existing entry's conclusion." Before writing the new entry, read the body of every related hit Stage 1 surfaced (not just its class name) and check whether the new finding narrows, overrides, or reverses that hit's Prevention rule. A hit is "related" when it targets the same object of action (e.g. the same tool, the same category of Plane object, the same command class) even under a different class slug.
   - **On conflict found**: do not silently add the new entry as an unrelated class. Add a scope qualifier to *both* entries so their Prevention rules no longer overlap unconditionally (e.g. "this rule applies to X, not to Y — see class `<other-slug>` for Y"). State the narrowed scope in the new entry's Prevention section explicitly, and append a one-line scope note to the older entry if editing it in place is feasible.
   - **Self-check (before finalizing Step 2's per-mistake write)**: does this entry's Prevention rule use a general/unqualified subject (e.g. "Plane" rather than "Plane Issue") where a related HOT entry already covers a narrower or broader case of the same subject? If yes, and the two rules would give opposite guidance for some sub-case, that sub-case must be named explicitly in both entries.

**Why retrospect.md also needs separate enforcement**:
- `fix.md` Step 1 is exclusive to the `fix:` trigger path
- retrospect is invoked via `/cleanup run` (through claudify improve → `Skill("fa")`) or a direct `/fa` call — a separate entry point
...
**Storage tool priority** (same as cleanup's `run.md` Step 5):
...
| # | Don't | Do |
|---|-------------|-----------------|
| 1 | Write "if more than 5, auto-run" as text without actually calling Skill() | Always confirm the count via grep → call `Skill("fa", "fa-prune")` immediately if exceeded |
...
| # | Don't | Do |
|---|-------------|-----------------|
| 1 | Add a "Violation case (YYYY-MM-DD, Nth occurrence)" subsection to the same file after strengthening a rule | Skills/rules contain **only rules (Don't/Do + self-checks)**. Violation cases are recorded to failed-attempts.md (HOT) via `/fa` |
