...
|---------|----------------|--------------------------------|
| `pm` | move → format → sync → priority → flowchart-sync (incl. pinned-mission liveness check) → **REPEAT cadence check** (run all due REPEAT items) → **Plan Drafts trigger scan** (surface, never author — see [draft.md](./draft.md) "Role ownership") → **Task Allocation & Channel Dispatch** (categorize triaged backlog into In-Session, Orca, Deep Tasks, Clawo, User Decision channels) | — |
...
**pm-profile task allocation & channel dispatch handoff (HARD STOP)**: when a `pm` role-profile default-invocation pipeline finishes (through the REPEAT cadence check and Plan Drafts scan), concluding the run with bookkeeping alone, an open-ended "what's next" prompt, or passively surfacing only a single lightweight candidate is strictly prohibited (`HARD STOP`). The core identity of the PM role is active work distribution. The PM agent MUST synthesize the live `sync`/`priority` triage results into a comprehensive **Task Allocation Matrix across all 5 execution channels**:
1. **Channel 1 (In-Session Direct Execution)**: simple P2/P3 single-file mechanical edits, documentation updates, or quick parameter fixes suitable for immediate turn execution without worktree isolation.
2. **Channel 2 (Orca Split Terminal / Worktree)**: P0/P1 multi-file implementations, TDD cycles, or isolated feature/fix branches requiring dedicated worktree and session isolation (`wsl -- claude --model <model>`).
3. **Channel 3 (Deep Tasks Audit Queue)**: architectural, ontology, multi-agent topology, or large-scale design evaluations requiring deep reasoning auditor models (`fable` / `opus`).
4. **Channel 4 (Clawo / Server Runtime)**: persistent background daemons, webhook listeners, or heavy batch RAG/archive sweeps.
5. **Channel 5 (External Wait / User Decision)**: blockers requiring explicit human infrastructure decisions, policy choices, or remote device access.

The agent MUST physically emit this structured Task Allocation Matrix in the visible response output and present clear channel-level dispatch options in `AskUserQuestion`, enabling immediate progression of high-priority work across roles.
