...
|---------|----------------|--------------------------------|
| `pm` | move → format → sync → priority → flowchart-sync → **REPEAT cadence check** (run all due REPEAT items) → **Plan Drafts trigger scan** → **Task Allocation & Channel Dispatch** (categorize triaged backlog into In-Session, Orca, Deep Tasks, Clawo, User Decision channels) | — |
