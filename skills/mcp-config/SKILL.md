...
metadata:
  ...
  version: "0.1.2"
description: |
  MCP server configuration and diagnostics. add - server registration with scope selection, move - change scope (project/local/user), format - JSON format per agent, catalog - common servers list, diagnostics - connection failure troubleshooting.
  Use when: "add MCP", "mcp-config", "add context7", "MCP format", "MCP server list", "move MCP", "MCP scope", "MCP connection failed", "MCP error", "Failed to reconnect", "code-mode not working".
---
