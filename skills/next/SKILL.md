...
**Both mandatory topic Reads are required, not either/or (HARD STOP)**: `ask-gates.md` (Step 0.3-0.7 skip/force gates) and `suggestion-patterns.md` (per-context templates + the Context-usage gate's Live-check fallback) are two separate files covering two separate obligations. Reading one does not satisfy the other — a turn that reads `ask-gates.md` and proceeds straight to composing options without also reading `suggestion-patterns.md` has skipped the file that holds the wrap-up/cleanup option rules, including the live context-usage re-check procedure. Case history: a cleanup option was offered with qualitative language ("session grew large") instead of a cited percentage, because `suggestion-patterns.md`'s "Live-check fallback" (direct `context-usage-inject.sh` invocation when the last injected reading is stale) was never read this turn — only `ask-gates.md` had been. Self-check before every `AskUserQuestion` call in this skill: did THIS turn Read both `ask-gates.md` AND `suggestion-patterns.md`, not just one?
