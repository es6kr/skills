...
- **Dual-Repo Sync Confirmation**: Always ask the user via `AskUserQuestion` whether to propagate changes across both configured wiki repositories (one per workspace).

## Overview & Workflow

1. **Format Conversion**: Convert rich documents (HWP/DOCX/PDF) to plain Markdown text while preserving structure and table formatting (utilizing `docxport` / `LibreOffice` / `pandoc` / `hwp5txt`).
...
6. **Dual-Repo Sync Confirmation**: Always ask the user via `AskUserQuestion` whether to propagate and commit wiki changes across both configured wiki repositories (one per workspace).
