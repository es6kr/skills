...
1. Collect registered hooks from the canonical inventory first (`Read` the registry at the private companion repo's `hook-registry.yaml`, relocated 2026-09-10; procedure: hook-kit's `registry.md` redirect), then compare against the live surfaces (settings.json `hooks`, plugin `hooks/hooks.json`) — ownership comes from the registry row, not from "a plugin hooks.json also lists it"
