...
# (.message.model). tail -n keeps the scan cheap on large transcripts; empty at
# the very first prompt of a session (no assistant entry yet) → line omitted.
resolve_model() {
  if [[ -n "$TRANSCRIPT_UNIX" && -f "$TRANSCRIPT_UNIX" ]]; then
    tail -n 400 "$TRANSCRIPT_UNIX" 2>/dev/null \
...
# asked. In Claude Code the session id is obtainable only through the harness
# (this injection or the /session id command) — the model cannot read it
# otherwise — so recording a session name/alias or a fabricated placeholder into
# fix_plan/RAG happens precisely when this value is absent. A one-time ~150-char
# inject is cheaper than a mis-recorded identifier that cannot be grepped back.
# (SessionStart has no `.prompt`, so only the session-only payload applies.)
...
#
# session-only injection (no message UUID): /session id, and /fix-plan. The
# fix-plan skill writes `(session <8-char prefix>)` into fix_plan completion
# notes, so it needs the session id at prompt time — but not the message UUID
# (only the RAG-import callers below need that). /cleanup is handled in the
# message-UUID branch because its 3-C.1 step calls the RAG import topic.
if [[ "$PROMPT" =~ ^/(claude-)?session[[:space:]]+id([[:space:]]|$) ]] || \
   [[ "$PROMPT" =~ ^/fix-plan($|[[:space:]]) ]]; then
  emit_session_only ""
  exit 0
fi
