...
TRANSCRIPT="$1"
# Normalize path based on runtime environment (WSL vs Git Bash) BEFORE checking file existence
normalize_path() {
  local p="$1"
  if grep -qi microsoft /proc/version 2>/dev/null; then
    # WSL environment: ensure path is /mnt/<drive>/...
    if [[ "$p" =~ ^([a-zA-Z]):[/\\](.*) ]]; then
    ...
    # Git Bash on Windows: convert /c/... to C:/...
    local drive="${BASH_REMATCH[1]^^}"
    local rest="${BASH_REMATCH[2]}"
    echo "${drive}:/${rest}"
    return
...
" "$search_dir" "$name_pat" "$maxdepth" 2>/dev/null
  ...
  if [ -z "$TRANSCRIPT" ] || [ ! -f "$TRANSCRIPT" ]; then
    ...
    for udir in /mnt/c/Users/*/.gemini/antigravity-cli/brain; do
      [ -d "$udir" ] && CANDIDATES+=("$udir")
    done
  ...
  if [ -z "$TRANSCRIPT" ] || [ ! -f "$TRANSCRIPT" ]; then
    ...
    for cdir in /mnt/c/Users/*/.claude/projects; do
      [ -d "$cdir" ] && CLAUDE_DIRS+=("$cdir")
    done
    for cdir in "${CLAUDE_DIRS[@]}"; do
      if [ -d "$cdir" ]; then
        FOUND=$(find_newest_transcript "$cdir" "*.jsonl")
        if [ -n "$FOUND" ] && [ -f "$FOUND" ]; then
          TRANSCRIPT="$FOUND"
          break
        fi
