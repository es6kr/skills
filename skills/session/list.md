...
Antigravity sessions live under `~/.gemini/<runtime>/brain/<uuid>/`, not `~/.claude/projects/` — and Antigravity is **two independent runtimes** with separate brain folders: the IDE app (`antigravity-ide`) and the CLI (`antigravity-cli`, invoked as `agy`). Neither shares sessions with the other, so enumerate both and merge:

```bash
for root in antigravity-ide antigravity-cli; do
  for dir in ~/.gemini/$root/brain/*/; do
    uuid=$(basename "$dir")
    t="$dir/.system_generated/logs/transcript.jsonl"
    [[ -f "$t" ]] || continue
    ls -l --time-style=full-iso "$t" | awk -v id="$uuid" -v r="$root" '{printf "| `%s` | %s | %s %s | %s |\n", id, r, $6, $7, $5}'
  done
done | sort -k3 -r
```
...
Output header:
```markdown
| Session UUID | Runtime | mtime | Size (bytes) |
|--------------|---------|-------|---------------|
```
